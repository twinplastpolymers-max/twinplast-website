...
import { HomepageSection, MarketsSectionContent } from '@/types';
...
export function MarketsSection({ initialSection }: { initialSection?: HomepageSection }) {
  ...
  const [regions, setRegions] = useState<string[]>((data.regions as string[]) || []);

  const handleSave = async () => {
    setIsSaving(true);
    try {
      const payload: MarketsSectionContent = { heading, content, regions };
      const { error } = await supabase
        .from('homepage_sections')
        .upsert({ section_key: 'markets', content: payload }, { onConflict: 'section_key' });
      if (error) throw error;
      toast.success('Saved', 'Markets section updated.');
    } catch (err) {
  ...
  return (
    <div className="space-y-5 w-full">
      <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Edit the heading, description, and market regions list.</p>
      </div>
      ...
      <div>
        ...
        <textarea id="mkt-content" rows={4} value={content} onChange={(e) => setContent(e.target.value)}
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
            Market Regions ({regions.length})
          </span>
          <button type="button" onClick={() => setRegions([...regions, 'New Region'])}
          ...
          {regions.map((reg, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              <input type="text" value={reg}
                onChange={(e) => { const u = [...regions]; u[idx] = e.target.value; setRegions(u); }}
              ...
              <button type="button" onClick={() => setRegions(regions.filter((_, i) => i !== idx))}
